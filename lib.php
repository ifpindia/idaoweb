...
{
	print "\n<rect id=\"recttooltip\" x=\"-100\" y=\"-100\" width=\"55\" height=\"6\" fill-opacity=\"0.4\" pointer-events=\"none\"/>";
	print "\n<text id=\"textboxid\" x=\"-100\" y=\"-100\" pointer-events=\"none\"> </text>";                 
	print "\n</a>";
	print "\n</svg>";
}
...
function calculateper()
{
	...
	//echo "<p class='result'>" . $ntopcount . " species at " . $topval . "%</p>";
	return $outputarr;
}

function formtargeturl($key)
{
	$targeturl = "http://www.biotik.org/india/species/";	
	$url = $targeturl . substr($key, 0,1) . "/" . $key ."/";
	$url = strtolower($url . $key . "_en.html");
	return $url;
}
